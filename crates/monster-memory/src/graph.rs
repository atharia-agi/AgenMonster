...
impl KnowledgeGraph {
    ...
    pub fn traverse(
        ...
        start_id: i64,
        max_depth: usize,
    ) -> anyhow::Result<Vec<(i64, usize, String)>> {
...
mod tests {
    ...
    fn test_add_and_retrieve_edges() {
        ...
        db.execute(
            "INSERT INTO memories (id, tier, content) VALUES (1, 'Hot', 'Test memory 1')",
            [],
        )
        .unwrap();
        db.execute(
            "INSERT INTO memories (id, tier, content) VALUES (2, 'Hot', 'Test memory 2')",
            [],
        )
        .unwrap();
    ...
    fn test_traverse() {
        ...
        db.execute(
            "INSERT INTO memories (id, tier, content) VALUES (1, 'Hot', 'A')",
            [],
        )
        .unwrap();
        db.execute(
            "INSERT INTO memories (id, tier, content) VALUES (2, 'Hot', 'B')",
            [],
        )
        .unwrap();
        db.execute(
            "INSERT INTO memories (id, tier, content) VALUES (3, 'Hot', 'C')",
            [],
        )
        .unwrap();
    ...
    fn test_stats() {
        ...
        assert!(stats
            .relations
            .iter()
            .any(|(r, c)| r == "relates_to" && *c == 1));
        assert!(stats
            .relations
            .iter()
            .any(|(r, c)| r == "caused_by" && *c == 1));
    }
}
