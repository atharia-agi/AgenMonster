...
impl Monitor {
    pub fn new() -> Self {
        Self {
            ...
            ticks: 0, energy_spent: 0, skills_learned: 0,
            llm_calls: 0, errors: 0, memories_stored: 0, tool_calls: 0,
        }
    }

    pub fn tick(&mut self) { self.ticks += 1; }
    pub fn record_llm_call(&mut self) { self.llm_calls += 1; }
    pub fn record_error(&mut self) { self.errors += 1; }
    pub fn record_tool_call(&mut self) { self.tool_calls += 1; }
    pub fn record_skill_learned(&mut self) { self.skills_learned += 1; }
    pub fn record_memory_stored(&mut self) { self.memories_stored += 1; }
    ...
    pub fn health_check(&self) -> HealthReport {
        HealthReport {
            ...
            errors: self.errors,
            error_rate: if self.llm_calls > 0 { self.errors as f32 / self.llm_calls as f32 } else { 0.0 },
