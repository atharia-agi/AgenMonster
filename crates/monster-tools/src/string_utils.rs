...
impl StringUtilsTool {
    ...
    pub fn description(&self) -> &str {
        "Transform text: uppercase, lowercase, trim, reverse, length, count_words."
    }
        ...
        let result = match op {
            ...
            "capitalize" => {
                ...
                match chars.next() {
                    ...
                    Some(f) => {
                        serde_json::json!({"result": format!("{}{}", f.to_uppercase(), chars.as_str())})
                    }
                }
            }
            "snake_case" => {
                let snake: String = input
                    .chars()
                    .enumerate()
                    .map(|(i, c)| {
                ...
                let camel: String = words
                    ...
                    .map(|(i, w)| {
                        ...
                        match chars.next() {
                            ...
                            Some(f) => {
                                if i == 0 {
                                    format!("{}{}", f.to_lowercase(), chars.as_str())
                                } else {
                                    format!("{}{}", f.to_uppercase(), chars.as_str())
                                }
                            }
                        }
                    })
...
mod tests {
    ...
    fn test_uppercase() {
        ...
        let result = tool
            .execute(&serde_json::json!({"input": "hello", "operation": "uppercase"}))
            .unwrap();
    ...
    fn test_reverse() {
        let tool = StringUtilsTool::new();
        let result = tool
            .execute(&serde_json::json!({"input": "abc", "operation": "reverse"}))
            .unwrap();
    ...
    fn test_count_words() {
        let tool = StringUtilsTool::new();
        let result = tool
            .execute(&serde_json::json!({"input": "hello world foo", "operation": "count_words"}))
            .unwrap();
