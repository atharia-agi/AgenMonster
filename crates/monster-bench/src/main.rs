use monster_bench::{BenchResult, BenchTracker};
...
fn bench_bus_publish(n: u32) -> f64 {
    ...
    let bus = monster_bus::Bus::new(monster_bus::BusConfig {
    ...
    rt.block_on(async {
        for _ in 0..n {
            let _ = bus.publish(
                monster_bus::topic::Topic::Telemetry,
                monster_bus::event::BusEvent::TelemetryTick,
            );
        }
    });
    start.elapsed().as_micros() as f64 / n as f64
...
fn bench_skill_match(n: u32) -> f64 {
    ...
    for _ in 0..n {
        for q in &queries {
            let _ = skills
                .iter()
                .find(|s| s.contains(q.split_whitespace().next().unwrap_or("")));
        }
    }
    start.elapsed().as_micros() as f64 / n as f64
}
